...
The dashboard is currently hard-coded for a single German-language deployment ("Vöhrum Vögelbeobachtung"). Making it whitelabel-ready so anyone can deploy their own instance with their own branding and language is the primary goal.

### 1a. Configuration-driven branding

- **Problem:** Site name ("Vöhrum Vögelbeobachtung"), tagline ("Live-Erkennungen aus der Wasserburg."), and "BirdNET-Go" header label are hard-coded in `App.tsx`.
- **Action:** Extract all branding into a central config file (`src/config/site.ts` or environment variables):
  - `siteName` – e.g. "Vöhrum Vögelbeobachtung" or "My Garden Birds"
  - `siteTagline` – e.g. "Live-Erkennungen aus der Wasserburg."
